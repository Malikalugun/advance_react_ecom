...
class Notification extends Component {
  ...
  render() {
    ...
    const MyView = NotificationList.map((Notifictaion, i) => {
      return (
        <Col className=" p-1 " md={6} lg={6} sm={12} xs={12}>
          <Card onClick={this.handleShow} className="notification-card">
            <Card.Body>
              <h6>{Notifictaion.title}</h6>
              <p className="py-1  px-0 text-primary m-0">
                <i className="fa  fa-bell"></i> Date: {Notifictaion.date} |
                Status: Unread
              </p>
              <Button
                data-title={Notifictaion.title}
                data-message={Notifictaion.message}
                data-date={Notifictaion.date}
